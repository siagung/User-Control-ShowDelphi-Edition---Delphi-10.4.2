      ...
      try
        ...
        begin
          ...
          FUserControl.MailUserControl.EnviaEmailAdicionaUsuario(vNome, vLogin,
            Encrypt(vNovaSenha, FUserControl.EncryptKey), vEmail,
            IntToStr(vPerfil), FUserControl.EncryptKey);
        end
...
begin
  ...
  try
    if ((ComboPerfil.ListSource.DataSet.RecordCount > 0) and
      VarIsNull(ComboPerfil.KeyValue)) then
      MessageDlg(FUserControl.UserSettings.CommonMessages.InvalidProfile,
        mtWarning, [mbOK], 0)
    else
    begin
      vNome := EditNome.Text;
      ...
      if FAltera then
      begin // alterar user
        FUserControl.ChangeUser(vNovoIDUsuario, vLogin, vNome, vEmail, vPerfil,
          vUserExpired, SpinExpira.Value, ComboStatus.ItemIndex, vPrivilegiado,
{$IFDEF DELPHI2006_UP}
...
{$ENDIF}
      ...
      begin // inclui user
        ...
        begin
          ...
          MessageDlg('Atenção, o campo login é obrigatório!', mtWarning,
            [mbOK], 0);
        ...
        begin
          ...
          begin
            ...
            FUserControl.AddUser(vLogin, vNovaSenha, vNome, vEmail, vPerfil,
              vUserExpired, SpinExpira.Value, vPrivilegiado,
{$IFDEF DELPHI2006_UP}
              ImageToBase64(iUserImage.Picture.Graphic)
{$ELSE}
              ''
...
begin
  ...
  try
    FOpenDialog.Filter :=
      'All|*.jpg; *.jpeg; *.gif; *.png|JPG|*.jpg; *.jpeg|GIF|*.gif';
    FOpenDialog.Options := [ofHideReadOnly, ofPathMustExist, ofFileMustExist,
      ofEnableSizing];
  ...
  begin
    if GetSize > ImageMaxSize then
      raise Exception.Create
        (Format(FUserControl.UserSettings.CommonMessages.ImageTooLarge,
        [IntToStr(ImageMaxSize)]));
